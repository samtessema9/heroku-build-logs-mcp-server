...
export function getHerokuAuthToken(env) {
    const token = env === "staging"
        ? process.env.HEROKU_AUTH_TOKEN_STAGING
        : process.env.HEROKU_AUTH_TOKEN_PRODUCTION;
    if (!token)
        throw new Error(`HEROKU_AUTH_TOKEN_${env.toUpperCase()} env var not set`);
    return token;
}
export async function fetchBuilds(appName, env) {
    ...
    if (!res.ok)
        throw new Error(`Failed to fetch builds: ${res.status} ${await res.text()}`);
    return res.json();
}
export async function fetchBuildLogs(outputStreamUrl) {
