...
import { z } from "zod";
import { fetchBuilds, fetchBuildLogs, HerokuEnv } from "./helper.js";
...
server.tool(
  ...
  "Get the latest build logs for a Heroku app (staging or production)",
  {
  ...
  async ({ appName, environment }) => {
    try {
      const env = (environment ?? "production") as HerokuEnv;
